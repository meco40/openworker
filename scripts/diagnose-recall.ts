...
try {
  ...
  for (const r of ftsResults) {
    console.log(
      `  [${r.role}] conv:${r.conversation_id.substring(0, 8)} | ${r.content.substring(0, 100)}`,
    );
  }
} catch (e) {
...
console.log('\n=== Knowledge Episodes ===');
const episodes = messagesDb.prepare('SELECT * FROM knowledge_episodes LIMIT 5').all() as Array<
  Record<string, unknown>
>;
console.log(
  `Total episodes: ${(messagesDb.prepare('SELECT COUNT(*) as cnt FROM knowledge_episodes').get() as { cnt: number }).cnt}`,
);
...
console.log('\n=== Memory Nodes ===');
const memNodes = messagesDb
  .prepare("SELECT * FROM memory_nodes WHERE content LIKE '%Regeln%' OR content LIKE '%regel%'")
...
for (const m of merkenMsgs) {
  console.log(
    `  [${m.role}] conv:${m.conversation_id.substring(0, 8)} | ${m.content.substring(0, 150)}`,
  );
}

// 5. Total messages count
const totalMsgs = messagesDb.prepare('SELECT COUNT(*) as cnt FROM messages').get() as {
  cnt: number;
};
