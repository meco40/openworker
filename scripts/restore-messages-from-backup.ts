...
const currentMessages = currentDb.prepare(
  'SELECT * FROM messages WHERE conversation_id = ? ORDER BY seq ASC'
...
if (currentMessages.length > 0) {
  ...
  const updateSeq = currentDb.prepare(
    'UPDATE messages SET seq = seq + ? WHERE id = ?'
  );
...
const oldContext = backupDb.prepare(
...
if (oldContext) {
  ...
  const existingContext = currentDb.prepare(
    'SELECT * FROM conversation_context WHERE conversation_id = ?'
  ).get(NEW_CONV_ID);
  ...
  if (existingContext) {
    ...
    ).run(oldContext.summary_text, oldContext.summary_upto_seq, new Date().toISOString(), NEW_CONV_ID);
    ...
    ).run(NEW_CONV_ID, oldContext.summary_text, oldContext.summary_upto_seq, new Date().toISOString());
...
// 8. Verify
const totalMessages = currentDb.prepare(
...
const ftsCheck = currentDb.prepare(`
  SELECT COUNT(*) as cnt
  FROM messages m
...
const merkenMsg = currentDb.prepare(`
