...
console.log('\n=== Step 5: Test Delete ===');
try {
  const conv = db.prepare('SELECT * FROM conversations WHERE id = ?').get(CONV_ID) as
  ...
  if (!conv) {
    ...
    db.prepare('DELETE FROM knowledge_ingestion_checkpoints WHERE conversation_id = ?').run(
      CONV_ID,
    );
