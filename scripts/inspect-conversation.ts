...
const msgCount = db.prepare('SELECT COUNT(*) as cnt FROM messages WHERE conversation_id = ?').get(convId) as { cnt: number };
...
for (const t of tables) {
  const fks = db.pragma(`foreign_key_list(${t.name})`) as Array<{ table: string; from: string; to: string }>;
  for (const fk of fks) {
    if (fk.table === 'conversations') {
      // Check if this table has entries referencing our conversation
      try {
        const count = db.prepare(`SELECT COUNT(*) as cnt FROM "${t.name}" WHERE "${fk.from}" = ?`).get(convId) as { cnt: number };
...
for (const t of tables) {
  try {
    ...
    if (hasConvCol) {
      const count = db.prepare(`SELECT COUNT(*) as cnt FROM "${t.name}" WHERE conversation_id = ?`).get(convId) as { cnt: number };
      if (count.cnt > 0) {
