...
| OpenCode                | API Key       | Nein  | `https://opencode.ai/zen/v1`               | OpenCode Zen; dynamischer `/models`-Katalog; Chat-Completions-Modelle |
...
## OpenCode-Integrationshinweis

OpenCode Zen authentifiziert per API-Key (`Authorization: Bearer ...`) und stellt
den dynamischen Katalog unter `GET https://opencode.ai/zen/v1/models` bereit.
Der Model Hub verwendet für Chat-Requests den OpenAI-kompatiblen Endpoint
`/chat/completions`. OpenCode dokumentiert zusätzlich modellabhängige
`/responses`- und `/messages`-Endpoints; diese Wire-APIs sind im generischen
Model-Hub-Adapter noch nicht enthalten und dürfen daher nicht als vollständig
kompatibel vorausgesetzt werden.
