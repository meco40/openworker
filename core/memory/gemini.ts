...
export const memoryStoreSpec: FunctionDeclaration = {
  ...
  description: "INTERNAL CORE: Persist critical information, user preferences, or 'lessons learned' to the system's long-term memory graph.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      type: { 
        type: Type.STRING, 
        enum: ["fact", "preference", "avoidance", "lesson", "personality_trait", "workflow_pattern"],
...
export const memoryRecallSpec: FunctionDeclaration = {
  ...
  description: "INTERNAL CORE: Retrieve historical data or context from the system's long-term memory graph based on a semantic query.",
  parameters: {
    type: Type.OBJECT,
    properties: {
...
export const taskScheduleSpec: FunctionDeclaration = {
  ...
  description: "Schedule a reminder or a proactive message for a specific future time. Use this for appointments, deadlines, or when the user asks to be reminded.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      time_iso: { type: Type.STRING, description: "The exact ISO 8601 timestamp for the reminder (e.g., 2025-05-20T10:00:00Z)." },
