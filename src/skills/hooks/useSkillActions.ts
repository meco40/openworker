...
export function useSkillActions(
  ...
  getMissingRequiredConfigs: (skillId: string) => import('@/skills/runtime-config-client').SkillRuntimeConfigStatus[],
): import('@/skills/components/registry/types').UseSkillActionsReturn {
  const [actionError, setActionError] = useState('');

  const handleToggleInstall = useCallback(async (id: string) => {
  ...
  const handleInstall = useCallback(async (params: {
    tab: InstallTab;
    value: string;
    beginClawHubAction: () => boolean;
    endClawHubAction: () => void;
    ...
    const { tab, value, beginClawHubAction, endClawHubAction, refreshClawHubInstalled, setShowModal, setInstallValue } = params;
