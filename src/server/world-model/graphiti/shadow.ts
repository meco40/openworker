...
export async function insertShadowEdge(input: ShadowEdgeInput): Promise<void> {
  ...
  await db.query(
    `INSERT INTO world_model_graphiti_shadow
      (source_outbox_event_id, user_id, persona_id, workspace_id, source_entity, target_entity,
       relation_type, confidence, evidence_observation_id, source_aggregate, valid_from, valid_to,
       shadow_episode)
...
export function createGraphitiShadowHandler() {
  return async (event: OutboxEvent): Promise<void> => {
    ...
    await insertShadowEdge({
      ...
      personaId,
      workspaceId,
      sourceAggregate: event.aggregateType,
      sourceEntity,
      targetEntity,
...
export async function countShadowEdges(
  userId?: string,
  personaId?: string,
  workspaceId?: string,
): Promise<number> {
