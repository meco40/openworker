...
export function toMemoryNode(record: Mem0MemoryRecord): MemoryNode {
  ...
  const memoryProvider =
    metadata.memoryProvider === 'postgres' ||
    metadata.memoryProvider === 'sqlite' ||
    metadata.memoryProvider === 'mem0'
      ? metadata.memoryProvider
      : 'mem0';
  return {
    id: record.id,
    ...
    metadata: {
      ...
      source:
        metadata.source === 'postgres' || metadata.source === 'sqlite' || metadata.source === 'mem0'
          ? metadata.source
          : memoryProvider,
      memoryProvider,
      lastVerified:
