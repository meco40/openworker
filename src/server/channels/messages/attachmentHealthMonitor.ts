...
export function evaluateAttachmentHealthSnapshot(
  ...
  const entries: Array<{ level: 'info' | 'warn'; message: string; metadata: Record<string, unknown> }> = [];
...
function resolveIntervalMs(): number {
  const raw = Number.parseInt(String(process.env.ATTACHMENT_CONSISTENCY_CHECK_INTERVAL_MS || ''), 10);
