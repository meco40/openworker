...
export class MessageService {
  ...
  private async buildRecallContext(
    ...
    if (knowledgeConfig.layerEnabled && knowledgeConfig.retrievalEnabled) {
      try {
        const knowledgeContext = getKnowledgeRetrievalService().buildRecallContext({
          userId: memoryUserId,
          personaId: conversation.personaId,
          query: userInput,
        });
        const normalizedKnowledgeContext = normalizeMemoryContext(knowledgeContext || '');
  ...
  private async maybeStoreKnowledgeArtifacts(
    ...
    try {
      getKnowledgeIngestionService().ingestConversationWindow({
        conversationId: conversation.id,
