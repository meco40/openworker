  ...
  },
  {
    ...
    up: (db) => {
      ...
      if (!tasksInfo.some((col) => col.name === 'workspace_id')) {
        db.exec(
          `ALTER TABLE tasks ADD COLUMN workspace_id TEXT DEFAULT 'default' REFERENCES workspaces(id)`,
        );
      ...
      if (!agentsInfo.some((col) => col.name === 'workspace_id')) {
        db.exec(
          `ALTER TABLE agents ADD COLUMN workspace_id TEXT DEFAULT 'default' REFERENCES workspaces(id)`,
        );
  ...
  },
  {
    ...
    up: (db) => {
      ...
      if (taskSchema && !taskSchema.sql.includes("'planning'")) {
        console.log(
          '[Migration 003] Note: tasks table needs planning status - will be handled by schema recreation on fresh dbs',
        );
      }
    },
  },
...
export function getMigrationStatus(db: Database.Database): {
  ...
  const applied = (
    db.prepare('SELECT id FROM _migrations ORDER BY id').all() as { id: string }[]
  ).map((m) => m.id);
