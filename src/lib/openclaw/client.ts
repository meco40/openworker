...
const globalProcessedEvents = (globalThis as unknown as Record<string, Map<string, number>>)[
  GLOBAL_EVENT_CACHE_KEY
];
...
export class OpenClawClient extends EventEmitter {
  ...
  private deviceIdentity: { deviceId: string; publicKeyPem: string; privateKeyPem: string } | null =
    null;
   ...
   */
  private generateEventId(data: unknown): string {
    const event =
  ...
  private performCacheCleanup(): void {
    ...
    if (globalProcessedEvents.size > this.MAX_PROCESSED_EVENTS) {
      const entriesToRemove =
        globalProcessedEvents.size - this.MAX_PROCESSED_EVENTS + this.CLEANUP_THRESHOLD;

      // Sort by access time (oldest first) and remove
      const sortedEntries = Array.from(globalProcessedEvents.entries()).sort((a, b) => a[1] - b[1]);
    ...
    if (removed > 0) {
      console.log(
        `[OpenClaw] Cache cleanup: removed ${removed} entries (size: ${initialSize} -> ${globalProcessedEvents.size})`,
      );
    }
  }
  ...
  private startPeriodicCleanup(): void {
    ...
    if (!(GLOBAL_CACHE_CLEANUP_KEY in globalThis)) {
      ...
      console.log(
        '[OpenClaw] Started periodic cache cleanup (interval:',
        this.PERIODIC_CLEANUP_INTERVAL_MS,
        'ms)',
      );
    }
    ...
    this.periodicCleanupTimer = (globalThis as unknown as Record<string, NodeJS.Timeout>)[
      GLOBAL_CACHE_CLEANUP_KEY
    ];
  }
    ...
    this.connecting = new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onclose = (event) => {
          ...
          console.log(
            `[OpenClaw] Disconnected from Gateway (code: ${event.code}, reason: "${event.reason}", wasClean: ${event.wasClean})`,
          );
        ...
        const messageHandler = (event: MessageEvent) => {
          try {
            ...
            const isRpcResponse =
              data.type === 'res' || (data.id !== undefined && this.pendingRequests.has(data.id));
            ...
            console.log(
              '[OpenClaw] Received:',
              data.type === 'res'
                ? `res:${String(data.id).slice(0, 8)}`
                : this.generateEventId(data).slice(0, 16),
            );
