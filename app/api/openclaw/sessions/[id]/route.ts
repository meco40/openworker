...
export async function PATCH(request: Request, { params }: RouteParams) {
  try {
    ...
    const session = db
      ...
      .get(id) as OpenClawSessionRow | undefined;
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!session) {
      session = db.prepare('SELECT * FROM openclaw_sessions WHERE id = ?').get(id) as
        | OpenClawSessionRow
        | undefined;
    }
    ...
    // Broadcast deletion event only for task-bound sessions
    if (taskId) {
      broadcast({
        type: 'agent_completed',
        payload: {
