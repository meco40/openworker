...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const answerText =
      answer?.toLowerCase() === 'other' && otherText ? `Other: ${otherText}` : answer;
...
If planning is complete, respond with JSON:
{
  ...
  "execution_plan": {
    ...
    console.log(
      '[Planning Answer] Sending answer to OpenClaw, session:',
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Failed to send answer to orchestrator: ' + (sendError as Error).message },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: 'Failed to submit answer: ' + (error as Error).message },
      { status: 500 },
    );
  }
}
