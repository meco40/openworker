...
interface ParsedPlanningResult {
  status?: string;
  question?: string;
...
async function handlePlanningCompletion(
  ...
  if (firstAgentId) {
    ...
    if (currentTask?.assigned_agent_id) {
      console.log(
        '[Planning Poll] Task already assigned to',
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const messages: PlanningMessage[] = task.planning_messages
      ? (JSON.parse(task.planning_messages) as PlanningMessage[])
      : [];
    ...
    console.log(
      '[Planning Poll] Comparison: stored_assistant=',
      initialAssistantCount,
      'openclaw_assistant=',
      openclawMessages.length,
    );
    ...
    if (openclawMessages.length > initialAssistantCount) {
      ...
      for (const msg of newMessages) {
        console.log(
          '[Planning Poll] Processing new message, role:',
          msg.role,
          'content length:',
        ...
        if (msg.role === 'assistant') {
          ...
          const parsed = extractJSON(msg.content) as ParsedPlanningResult | null;
          ...
          if (parsed && parsed.status === 'complete') {
            ...
            const {
              ...
              parsed: fullParsed,
              dispatchError,
            } = await handlePlanningCompletion(taskId, parsed, messages);
      ...
      console.log(
        '[Planning Poll] Returning updates: currentQuestion =',
        currentQuestion ? 'YES' : 'NO',
      );

      // Update database
      run('UPDATE tasks SET planning_messages = ? WHERE id = ?', [
        JSON.stringify(messages),
        taskId,
      ]);
