...
describe('config editor validation mapping', () => {
  it('maps backend validation messages to known field paths', () => {
    expect(
      mapValidationMessageToFieldPath('gateway.port must be an integer between 1 and 65535.'),
    ).toBe('gateway.port');
    expect(mapValidationMessageToFieldPath('ui.timeFormat must be one of: 12h, 24h.')).toBe(
      'ui.timeFormat',
    );
  });
