...
const knowledgeBuildRecallContextMock = vi.hoisted(() =>
  vi.fn(() => 'Knowledge: Mittags Sauna mit Details zu Temperatur und Aufguss.'),
);
...
vi.mock('../../../src/server/knowledge/runtime', () => ({
  getKnowledgeRetrievalService: () => ({
    buildRecallContext: knowledgeBuildRecallContextMock,
  }),
}));
...
describe('MessageService knowledge recall integration', () => {
  beforeEach(() => {
    ...
    knowledgeBuildRecallContextMock.mockClear();
  ...
  it('injects knowledge context for retrospective sauna prompt before mem0 fallback', async () => {
    ...
    expect(knowledgeBuildRecallContextMock).toHaveBeenCalledTimes(1);
